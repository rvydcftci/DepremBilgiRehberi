...
const safeZoneController = {
    ...
    getNearestSafeZones: async (req, res) => {
        try {
            ...
            const nearestZones = await SafeZone.find({
                location: {
                    $near: {
                        $geometry: {
                            type: "Point",
            ...
            res.json(nearestZones);
        } catch (error) {
            console.error('En yakın güvenli bölgeler getirilemedi:', error);
